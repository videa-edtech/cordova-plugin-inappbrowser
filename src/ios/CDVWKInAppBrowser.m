...
{
    // Kiểm tra nếu URL bị nil
    if (url == nil) {
        NSLog(@"URL is nil. Cannot open.");
        return;
    }

    NSDictionary *options = @{};

    // Sử dụng API openURL mới với completionHandler
    [[UIApplication sharedApplication] openURL:url options:options completionHandler:^(BOOL success) {
        if (!success) {
            // Log lỗi nếu không mở được URL
            NSLog(@"Failed to open URL: %@", url);

            // Gửi thông báo qua NotificationCenter
